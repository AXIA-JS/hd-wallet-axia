...
import Axia from "../src/Axia";
import BIPPath from "bip32-path";

test("axia.getPublicKey", async () => {
  ...
  const result = await axia.getWalletExtendedPublicKey("m/44'/9000'/0'/1/0");
});

test("axia.getAppConfiguration", async () => {
  const transport = await openTransportReplayer(
    RecordStore.fromString(`
    => 8000000000
    <= 0001009000
  ...
  const axia = new Axia(transport);
  var configuration = await axia.getAppConfiguration();
...
test("axia.signTransaction", async () => {
  ...
  const result = await axia.signTransaction(
